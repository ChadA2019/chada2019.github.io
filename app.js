...
function renderReview(){
  reviewList.innerHTML=state.reviewQueue.length?state.reviewQueue.map((q,i)=>{
    const chosenCategory=q.chosenCategory||q.category;
    ...
    return `<article class="review-card">
      ...
      <div class="review-grid">
        ...
        <label>Chosen category
          <input data-i="${i}" data-k="chosenCategory" list="categoryList" value="${esc(chosenCategory)}">
        </label>
        <label>Subcategory
        ...
        <label>Asset<input data-i="${i}" data-k="asset" list="assetListData" value="${esc(q.asset||"")}"></label>
        <label>Accept<select data-i="${i}" data-k="accept">
  ...
  document.querySelectorAll("#reviewList [data-i]").forEach(el=>el.onchange=e=>{
    const q=state.reviewQueue[+e.target.dataset.i],k=e.target.dataset.k;
    q[k]=k==="accept"?e.target.value==="true":e.target.value;

    if(k==="chosenCategory"){
      const available=getSubcategories(q.chosenCategory);
      if(!available.includes(q.chosenSubcategory)){
