...
managedDialogs.forEach(dialog=>{
  ...
  dialog.addEventListener('close',()=>{if(form)form.dataset.dirty='false';});
});
...
const APP_INFO=Object.freeze({version:"9.9.2",build:"2026.07.24.020",release:"Version 9.9.2 makes Cancel reliable across forms and adds safe unsaved-change protection."});
...
function openNameManager({title,description="",label="Name",value="",saveText="Save",onSave}){
  ...
  markDialogClean(nameManagerDialog);nameManagerDialog.showModal();
...
function renderTransactions(){const q=upper(transactionSearch.value),cat=categoryFilter.value,asset=assetFilter.value,type=transactionTypeFilter?.value||"",rows=state.transactions.map((t,i)=>({...t,_i:i})).filter(t=>(!q||upper(`${t.description} ${t.merchant} ${t.tag} ${t.notes}`).includes(q))&&(!cat||t.category===cat)&&(!asset||t.asset===asset)&&(!type||(type==="income"&&Number(t.amount)>0)||(type==="expense"&&Number(t.amount)<0)||(type==="review"&&!t.reviewed))).sort((a,b)=>b.date.localeCompare(a.date));transactionsBody.innerHTML=rows.map(t=>`<tr><td>${esc(t.date)}</td><td>${esc(t.description)}</td><td>${money(t.amount)}</td><td>${esc(t.category)}</td><td>${esc(t.asset||"")}</td><td>${t.taxDeductible?"Yes":"No"}</td><td><span class="status ${t.reviewed?"reviewed":"pending"}">${t.reviewed?"Reviewed":"Review"}</span></td><td><button class="link-btn edit-tx" data-i="${t._i}">Edit</button></td></tr>`).join("");document.querySelectorAll(".edit-tx").forEach(b=>b.onclick=()=>openTransaction(+b.dataset.i))}
function openTransaction(i=null){const t=i===null?{date:localDateValue(),amount:"",description:"",category:"Uncategorised",subcategory:"Review Required",asset:"",taxDeductible:false,tag:"",notes:""}:state.transactions[i];txIndex.value=i===null?"":i;transactionDialogTitle.textContent=i===null?"Add Transaction":"Edit Transaction";txDate.value=t.date;txAmount.value=t.amount;txDescription.value=t.description;fillCategorySelect(txCategory,txSubcategory,t.category,t.subcategory||"");fillAssetSelect(txAsset,t.asset||"");txTax.value=String(!!t.taxDeductible);txTag.value=t.tag||"";txNotes.value=t.notes||"";markDialogClean(transactionDialog);transactionDialog.showModal()}
...
function makeKpiShortcut(id,action){
  const card=document.getElementById(id);
  if(!card)return;
  const run=()=>{action();requestAnimationFrame(()=>window.scrollTo({top:0,behavior:"smooth"}));};
  card.addEventListener("click",run);
  card.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();run();}});
}
makeKpiShortcut("reviewKpiCard",()=>{rebuildReviewQueue();renderReview();activateView("review");requestAnimationFrame(()=>document.querySelector("#reviewList .review-card")?.scrollIntoView({block:"start",behavior:"smooth"}));});
makeKpiShortcut("receiptMatchKpiCard",()=>{renderReceipts();activateView("receipts");});
makeKpiShortcut("incomeKpiCard",()=>{transactionSearch.value="";categoryFilter.value="";assetFilter.value="";if(transactionTypeFilter)transactionTypeFilter.value="income";renderTransactions();activateView("transactions");});
makeKpiShortcut("expenseKpiCard",()=>{transactionSearch.value="";categoryFilter.value="";assetFilter.value="";if(transactionTypeFilter)transactionTypeFilter.value="expense";renderTransactions();activateView("transactions");});
makeKpiShortcut("netKpiCard",()=>{renderReports();activateView("reports");});
...
function openStatementChooser(){
  if(typeof importChoiceDialog.showModal==="function"){markDialogClean(importChoiceDialog);importChoiceDialog.showModal();}
...
applyReviewsBtn.onclick=learnAccepted;transactionSearch.oninput=renderTransactions;categoryFilter.onchange=renderTransactions;assetFilter.onchange=renderTransactions;if(transactionTypeFilter)transactionTypeFilter.onchange=renderTransactions;dashFrom.onchange=renderDashboard;dashTo.onchange=renderDashboard;fySelect.onchange=renderReports;
...
addRuleBtn.onclick=()=>{ruleForm.reset();fillCategorySelect(ruleCategory,ruleSubcategory,"Uncategorised","Review Required");fillAssetSelect(ruleAsset,"");markDialogClean(ruleDialog);ruleDialog.showModal()};saveRuleBtn.onclick=e=>{e.preventDefault();state.rules.unshift({pattern:upper(rulePattern.value),category:ruleCategory.value,subcategory:ruleSubcategory.value,asset:ruleAsset.value});ruleForm.reset();ruleDialog.close();saveState();renderAll()};
